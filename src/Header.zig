...
pub fn decode(allocator: std.mem.Allocator, reader: anytype) !Header {
    var r = Header{
...
pub fn encode(header: Header, writer: anytype) @TypeOf(writer).Error!void {
    try writer.print("Content-Length: {d}\r\n", .{header.content_length});
    if (header.content_type) |ct|
